...
RTSP_Channel::RTSP_Channel(RTSP * rtsp): QObject(rtsp), _rtsp(rtsp), _id(_rtsp->channelsCount())
{
  ...
  _streamer =new RTSPStream();
  _streamerThread =new QThread(this);
  _streamer->moveToThread(_streamerThread);

  //-- Как только поток запустится запускаем стример
  connect(_streamerThread, &QThread::started, _streamer, &RTSPStream::start);
  //-- Как только стример сам пожелает завершится завершаем сначала поток
  connect(_streamer, &RTSPStream::completed, _streamerThread, &QThread::quit);

  connect(_streamer, &RTSPStream::connected, this, &RTSP_Channel::connected, Qt::QueuedConnection);
  connect(_streamer, &RTSPStream::disconnected, this, &RTSP_Channel::disconnected, Qt::QueuedConnection);
  connect(_streamer, &RTSPStream::errored, this, &RTSP_Channel::onStreamError, Qt::QueuedConnection);
...
RTSPStream *RTSP_Channel::getStreamer()
{
  return _streamer;
}
