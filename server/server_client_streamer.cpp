...
#include "rtp/rtp_packet_h265.h"
#include "rtp/rtp_packet_h265_unit_fu.h"
    ...
    switch ( packet->payloadType() ) {
      case 98: { //-- h265
        ...
        if ( rtph265->isFUUnit() ) { //-- Фрагментируем FU пакет на несколько, что бы влезли в BlockSize
          ...
          RTP_Packet_H265_Unit_FU partRTPh265FU(QByteArray(rtph265FU->data().data(), rtph265->size()));

          const RTP_Packet_H265_Unit_FU::FU_PARTS srcFUPart =rtph265FU->fuPart(); //-- Запоминаем какая часть была изначально, т.к. мы будет добавлять новые нужно сохранить правильный порядок
